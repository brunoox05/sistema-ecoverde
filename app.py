...
st.markdown("""
    ...
    /* Rodapé Fixo (Esquerda para não bater no Manage App) */
    .footer { 
        position: fixed; 
        bottom: 10px; 
        left: 20px; 
...
def check_password():
    ...
    with col2:
        st.markdown("<br><br><h2 style='text-align: center; color: #548235;'>🌿 EcoVerde - Acesso Restrito</h2>", unsafe_allow_html=True)
        senha = st.text_input("Digite a senha de acesso:", type="password")
        if st.button("ENTRAR", use_container_width=True):
            if senha == "Ambiental2011@":
...
def limpar_banco_completo():
    conn = sqlite3.connect('dados_ric_oficial.db')
    try:
        conn.execute("DELETE FROM ped_ruas"); conn.execute("DELETE FROM peds")
        conn.execute("DELETE FROM empresa_ruas"); conn.execute("DELETE FROM ruas")
        conn.execute("DELETE FROM empresas"); conn.execute("DELETE FROM sqlite_sequence")
...
def excluir_rua(rua_id):
    conn = get_conexao()
    try:
        conn.execute("DELETE FROM empresa_ruas WHERE rua_id = ?", (rua_id,))
        conn.execute("DELETE FROM ped_ruas WHERE rua_id = ?", (rua_id,))
        conn.execute("DELETE FROM ruas WHERE id = ?", (rua_id,))
        conn.commit()
        return True
    except: return False
    finally: conn.close()
...
def excluir_empresa(empresa_id):
    conn = get_conexao()
    try:
        # Pega todas as ruas dessa empresa
        ruas = pd.read_sql(f"SELECT rua_id FROM empresa_ruas WHERE empresa_id={empresa_id}", conn)
        ids_ruas = ruas['rua_id'].tolist()
        
        # Deleta dependências
        for rid in ids_ruas:
            conn.execute("DELETE FROM ped_ruas WHERE rua_id=?", (rid,))
            conn.execute("DELETE FROM ruas WHERE id=?", (rid,))
            
        conn.execute("DELETE FROM peds WHERE empresa_id=?", (empresa_id,))
        conn.execute("DELETE FROM empresa_ruas WHERE empresa_id=?", (empresa_id,))
...
def criar_empresa(nome):
    conn = get_conexao()
    conn.execute("INSERT INTO empresas (nome, data_criacao) VALUES (?, ?)", (nome, datetime.now().strftime("%Y-%m-%d")))
    conn.commit(); conn.close()
...
def classificar_calcada(largura): return "Estreita (Inferior a 1,20m)" if largura < 1.2 else "Ampla (Superior a 1,20m)"
...
with st.sidebar:
    ...
    st.markdown("**Criado por Bruno Maia**") # Crédito visível
    ...
    # --- ZONA DE PERIGO (LÁ EM BAIXO) ---
    st.markdown("<br><br><br>", unsafe_allow_html=True) # Espaço
    with st.expander("⚠️ ZONA DE PERIGO"):
        st.warning("Isso apaga TODOS os dados!")
        senha_del = st.text_input("Senha Admin:", type="password", key="pwd_del")
        if st.button("LIMPAR TUDO (RESET)", type="primary"):
            if senha_del == "92709211":
...
if menu == "Nova Empresa":
    ...
    with c1: nome = st.text_input("Nome do Empreendimento / Cliente")
    with c2: 
    ...
    with t1:
        ...
        if estacionam == "Sim":
            col_est2.selectbox("Situação do Estacionamento?", ["Estacionam em local adequado", "Estacionam em local não adequado", "Estacionam em cima da calçada de forma irregular"], key="r_local_est")
        else: st.session_state.r_local_est = "N/A"
        
        col_pav1, col_pav2, col_pav3 = st.columns([1, 2, 2])
        col_pav1.radio("Possui Pavimento?", ["Sim", "Não"], horizontal=True, key="r_tem_pav")
        with col_pav2:
            if st.session_state.r_tem_pav == "Sim": st.selectbox("Tipo Pavimento", ["Asfalto", "Bloquete", "Concreto", "Poliedrico"], key="r_tipo_pav_sim")
            else: st.selectbox("Tipo Revestimento", ["Terra", "Cascalho", "Leito Natural"], key="r_tipo_pav_nao")
        with col_pav3: st.selectbox("Estado de Conservação", ["Bom", "Regular", "Ruim", "Péssimo"], key="r_estado_pav")
    ...
    with t2:
        st.subheader("Importar ruas")
        conn = get_conexao()
        q = f"SELECT * FROM ruas WHERE id NOT IN (SELECT rua_id FROM empresa_ruas WHERE empresa_id={empresa_id})"
        df_glob = pd.read_sql(q, conn); conn.close()
        if not df_glob.empty:
            c_imp1, c_imp2 = st.columns([3, 1], vertical_alignment="bottom")
            with c_imp1: sel = st.selectbox("Selecione a rua para importar:", df_glob['nome'] + " (ID: " + df_glob['id'].astype(str) + ")")
            with c_imp2:
                if st.button("📥 Importar Rua"):
                    id_imp = int(sel.split("ID: ")[1].replace(")",""))
                    if importar_rua(empresa_id, id_imp): st.success("Importada!"); st.rerun()
        else: st.warning("Não há ruas disponíveis para importar.")
    ...
    with c_top2:
        ...
        conn = get_conexao()
        ruas = pd.read_sql(f"SELECT r.id, r.nome FROM ruas r JOIN empresa_ruas er ON r.id=er.rua_id WHERE er.empresa_id={empresa_id}", conn); conn.close()
    ...
    st.title(f"📊 Relatório: {empresa_nome}")
    if st.button("📄 GERAR RELATÓRIO WORD", use_container_width=True, type="primary"):
        try:
            arq = gerar_docx(empresa_id, empresa_nome)
            with open(arq, "rb") as f: st.download_button("⬇️ BAIXAR ARQUIVO", f, file_name=arq, mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", icon="📥")
        except Exception as e: st.error(f"Erro: {e}")
    
    st.divider()
    
    # --- ÁREA DE LIXEIRA ---
    with st.expander("🗑️ Gerenciar Exclusões (Lixeiras)"):
        t_del1, t_del2 = st.tabs(["Excluir Ruas", "Excluir Projeto"])
        
        with t_del1:
            conn = get_conexao()
            df_r = pd.read_sql(f"SELECT r.id, r.nome FROM ruas r JOIN empresa_ruas er ON r.id=er.rua_id WHERE er.empresa_id={empresa_id}", conn); conn.close()
            if not df_r.empty:
                sel_del = st.selectbox("Selecione a rua para excluir:", df_r['nome'] + " (ID: " + df_r['id'].astype(str) + ")")
                if st.button("Confirmar Exclusão da Rua"):
                    rid_del = int(sel_del.split("ID: ")[1].replace(")",""))
                    if excluir_rua(rid_del): st.success("Rua excluída!"); st.rerun()
            else: st.info("Nenhuma rua cadastrada.")
            
        with t_del2:
            st.warning(f"Atenção: Isso excluirá o projeto '{empresa_nome}' e todos os seus dados!")
            if st.button("Confirmar Exclusão do Projeto"):
                if excluir_empresa(empresa_id): st.success("Projeto excluído!"); st.rerun()
